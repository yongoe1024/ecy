...
public class UploadServiceImpl implements UploadService {
    ...
    public void uploadChunk(FileChunkReq chunkReq) throws IOException {
        //分块的目录
        String chunkFileFolderPath = getChunkFilePath(chunkReq);
    ...
    public boolean mergeChunk(FileChunkReq chunkReq)  {
        String chunkFilePath = getChunkFilePath(chunkReq);
        String filePath = getFilePath(chunkReq);
        // 检查分片是否都存在
        if (checkChunks(chunkReq)) {
     ...
     * 检查分片是否都存在
     */
    private boolean checkChunks(FileChunkReq chunkReq) {
        try {
            for (int i = 0; i < chunkReq.getTotalChunks() ; i++) {
                File file = new File(Path.of(getChunkFilePath(chunkReq), String.valueOf(i)).toString());
                if (!file.exists()) {
    ...
    private synchronized void saveToRedis(FileChunkReq chunkReq) {
        ...
        if (map == null || map.size()==0) {
            ...
            map.put("filePath", getChunkFilePath(chunkReq));
        }
        Set<Integer> uploaded = (Set<Integer>) map.get("uploaded");
     ...
     * 得到文件的绝对路径
     */
    private String getFilePath(FileChunkReq fileChunkReq) {
        String fileName = fileChunkReq.getFileName();
        String suffix = fileName.substring(fileName.lastIndexOf("."));
        fileName = fileChunkReq.getMd5() + suffix;
     ...
     * 得到分块文件保存的目录
     */
    private String getChunkFilePath(FileChunkReq fileChunkReq) {
        return Path.of(getFullFilePath(), "chunks", fileChunkReq.getMd5()).toString();
    }
